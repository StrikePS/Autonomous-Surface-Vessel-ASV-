...
class DataPacketParser:
    ...
    def parse_data_packet(packet, xbus_data):
        ...
        bytes_offset = 0
        data_len_total = len(data_part)

        while bytes_offset + 3 <= data_len_total:

            data_id = data_part[bytes_offset:bytes_offset+2]
            data_len = data_part[bytes_offset+2]

            block_end = bytes_offset + 3 + data_len

            # 🚑 HARD SAFETY CHECK
            if block_end > data_len_total:
                # Drop rest of this packet safely
                return

            packet_data = data_part[bytes_offset+3:block_end]

            DataPacketParser.parse_mtdata2(xbus_data, data_id, packet_data)

            bytes_offset = block_end
